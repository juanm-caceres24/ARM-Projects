...
#define POTENTIOMETER_1_PIN 24 // P0.24
...
void configADC() {
	...
	PinCfg.Pinnum = POTENTIOMETER_1_PIN; // ADC on P0.24
	PINSEL_ConfigPin(&PinCfg);
...
void ADC_IRQHandler(){
	if (channel_selection == 0) {
		potentiometer_0_value = (LPC_ADC->ADDR0)>>2;
	} else {
		potentiometer_1_value = (LPC_ADC->ADDR1)>>2;
	}
	channel_selection = !channel_selection;

	ADC_ChannelCmd(LPC_ADC, channel_selection, ENABLE);
